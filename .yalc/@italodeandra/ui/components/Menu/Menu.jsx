...
const Item = function MenuItem({ className, icon, children, ...props }) {
    return (<HuiMenu.Item>
      {({ active }) => (<UnstyledButton {...props} type="button" className={clsx("flex w-full items-center text-left", active
                ? "bg-zinc-100 text-zinc-900 dark:bg-zinc-700/50 dark:text-zinc-100"
                : "text-zinc-700 dark:text-zinc-200", "block px-4 py-2 text-sm", className)}>
          {icon &&
                cloneElement(icon, {
...
const Menu = forwardRef(function Menu({ className, iconClassName, position = "right", children, label, button, buttonProps, unmount, menuItemsClassName, ...props }, ref) {
    ...
    return (<HuiMenu as="div" className={clsx("relative inline-block text-left", className)} ref={ref} {...props}>
      <div>
        ...
        as={Button} className={clsx("flex w-full", buttonProps?.className)} trailing={<ChevronDownIcon className={iconClassName}/>} {...buttonProps}>
            {label}
          </HuiMenu.Button>)}
      </div>

      <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
        <HuiMenu.Items unmount={unmount} className={clsx(defaultMenuItemsClassName, "absolute mt-2 min-w-[14rem]", {
