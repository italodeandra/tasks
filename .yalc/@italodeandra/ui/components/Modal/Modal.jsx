import { cloneElement, Fragment, useCallback, } from "react";
...
export default function Modal({ open, onClose, children, overlayClassName, panelClassName, dialogClassName, dialogOuterPanelClassName, dialogOverflowClassName, style, }) {
    const handleOnClose = useCallback(() => onClose?.(), [onClose]);
    return (<Transition.Root show={open} as={Fragment}>
      <Dialog as="div" className={clsx("relative z-10", dialogClassName)} onClose={handleOnClose} style={style}>
        <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0" enterTo="opacity-100" leave="ease-in duration-200" leaveFrom="opacity-100" leaveTo="opacity-0">
          <div className={clsx("fixed inset-0 bg-white/75 transition-opacity dark:bg-black/75", overlayClassName)}/>
        </Transition.Child>
...
function ModalContainer({ children, className, }) {
    return (<Stack className={clsx("!gap-4 px-4 pb-4 pt-5 sm:p-6", className)}>
      {children}
    </Stack>);
}
Modal.Title = ModalTitle;
function ModalTitle({ children, className, }) {
    return (<Dialog.Title as="h3" className={clsx("text-center text-lg font-medium leading-6 text-zinc-900 dark:text-zinc-100", className)}>
      {children}
    </Dialog.Title>);
}
Modal.Content = ModalContent;
function ModalContent({ children, className, }) {
    return (<div className={clsx("text-center text-sm text-zinc-500 dark:text-zinc-400", className)}>
      {children}
    </div>);
}
...
function ModalCloseButton({ className, ...props }) {
    return (<Button icon className={clsx("absolute right-2 top-2", className)} variant="text" {...props}>
      <XMarkIcon />
    </Button>);
}
